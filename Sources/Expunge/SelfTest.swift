...
enum SelfTest {
    ...
    static func run() -> Int32 {
        let checks: [Check] = [
            ...
            Check(name: "bundle id 通用末段不当别名（com.expunge.app 不下发 app）") {
                // 真机 bug：`--scan Expunge` 曾扫出 556 项 / 34.36 GB。
                // com.expunge.app 的末段 "app" 被下发给扫描器后，
                // ~/Library/Application Support 下几乎每个目录都命中
                // （含一个 458 MB 的无关 app 数据目录和一堆 com.apple.*）。
                // foreignKeywords 防不住 —— 那些目录不属于任何已装 app。
                let app = AppIdentity(bundlePath: "/Applications/Expunge.app", fileName: "Expunge",
                                      displayName: "Expunge", bundleId: "com.expunge.app",
                                      version: nil, aliases: [])
                let q = ScanQuery(raw: "Expunge", app: app)
                return app.bundleIdSuffix == nil
                    && !q.keywords.contains("app")
                    && !q.matches("Application Support")
                    && q.keywords.contains("expunge")
            },
            ...
            Check(name: "每个 skill 的 cli 都对应真实 CLI flag（防假成功）") {
                ...
                // Expunge CLI 命令。这里拿注册表里的 cli 和 --help 对表。
                let usage = ExpungeApp.usage
            ...
            Check(name: "ChatStore 真机往返 + 损坏 / 半截 / 缺失文件一律静默降级为空") {
                ...
                let fm = FileManager.default
                let path = "\(NSHomeDirectory())/Library/Application Support/Expunge/askai-history.json"
            ...
            Check(name: "--help 里印的版本号跟得上发版") {
                ...
                // 现在两处共用 ExpungeApp.version，这条只保证它不是占位符
                // 且形如数字版本号（允许 "1.0" 或 "1.0.0"）。
                let v = ExpungeApp.version
