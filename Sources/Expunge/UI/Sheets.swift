...
struct FeedbackSheet: View {
                    ...
                    Toggle(isOn: $includeDiagnostics) {
                        VStack(alignment: .leading, spacing: 2) {
                            Text(L10n.t("附带诊断信息", "Include diagnostics"))
                                .font(.system(size: 12))
                            Text(L10n.t("仅包含 macOS 版本、Expunge 版本与命中的路径；用户名会被打码，绝不上传文件内容。",
                                        "Only macOS version, Expunge version and the matched path. Your username is masked and no file contents are sent."))
                                .font(.system(size: 10.5))
                                .foregroundStyle(.tertiary)
    ...
    private func bodyText() -> String {
        ...
        if includeDiagnostics {
            ...
            lines.append("- Expunge \(ExpungeApp.version)")
