...
/// 「关于 Expunge」弹窗（替代系统标准关于面板，内容可控、与设计系统一致）。
struct AboutSheet: View {
    @Environment(\.dismiss) private var dismiss
    @Environment(\.openURL) private var openURL

    private let repoURL = URL(string: "https://github.com/hwd8080/expunge-mac-uninstaller")!
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            VStack(alignment: .leading, spacing: 8) {
                Text(L10n.t("Expunge 帮你把 app 连同它的依赖、缓存、配置和残留痕迹一起清掉，",
                            "Expunge removes an app together with its dependencies, caches, "
