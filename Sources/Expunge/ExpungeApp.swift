...
struct ExpungeApp: App {
    ...
    nonisolated static let usage = """
        Expunge \(Self.version) —— Mac 深度卸载工具 / deep uninstaller for macOS

        用法 / Usage:
          Expunge                                  启动图形界面 / launch the GUI
          Expunge --list                           列出已安装 app / list installed apps
          Expunge --scan <名称>                     只扫描，不删除 / scan only
          Expunge --uninstall <名称> [选项]          扫描并卸载 / scan and uninstall
          Expunge --orphans                        查找孤儿残留 / find orphaned leftovers
          Expunge --history                        卸载历史 / uninstall history
          Expunge --reset <名称>                    重置 app（保留程序本体）/ reset an app
          Expunge --skills                         列出 Agent 可调用的 skill / list agent skills
          Expunge --agent "自然语言目标" [--model <档名>]   启动 Agent：自动决定调哪些 skill / launch the agent
          Expunge --skill <名称> [--args '{...}']    直接调一个 skill / call a skill directly
          Expunge --ps [--filter background|user|system] [--keyword <名>] [--limit <N>]   列出进程 / list processes
          Expunge --review [--scope apps|leftovers]   复核删除清单 / review a removal plan
          Expunge --self-test                      运行自检 / run self-test
          Expunge --help                           显示本帮助 / show this help
